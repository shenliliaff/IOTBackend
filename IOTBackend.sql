...
`id` int primary key unique NOT NULL AUTO_INCREMENT,   
`user_id` int NOT NULL unique COMMENT '用户id',   
`role_id` int NOT NULL unique COMMENT '角色id', 
...
`id` int primary key unique NOT NULL AUTO_INCREMENT,   
`permission_id` int NOT NULL unique COMMENT '权限id',   
`role_id` int NOT NULL unique COMMENT '角色id', 
...
`id` int primary key unique NOT NULL AUTO_INCREMENT,   
`group_id` int unique COMMENT '用户组id', 
`role_id` int unique COMMENT '角色id',   
...
`device_type_id` varchar(128) NOT NULL COMMENT '区域id',
`is_in_use` tinyint NOT NULL COMMENT '是否有效',   
`creator` varchar(128) NOT NULL COMMENT '创建者',   
`create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',   
`modifier` varchar(128) NOT NULL COMMENT '用户名称',   
...
`id` int primary key unique NOT NULL AUTO_INCREMENT,   
`device_sn` int NOT NULL COMMENT '设备sn',  
...
`venue_id` varchar(128) NULL COMMENT '场馆id',
`is_in_use` tinyint NOT NULL COMMENT '是否有效',   
`creator` varchar(128) NOT NULL COMMENT '创建者',   
`create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',   
`modifier` varchar(128) NOT NULL COMMENT '用户名称',   
...
`key_content` varchar(500) NULL COMMENT '钥匙内容',   
